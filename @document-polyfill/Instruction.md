...
You should write all domains according to their decreasing importance in the hierarchy, including the top-level domain, i.e. in the reverse order than their occurrence in the address; each of them should be separated by a dash:`-`. If the dash is part of a domain name, replace it with a `^`.
